...
body field checked against a second shared secret, `services/bridge/.approval-token` (zone C
only, generated by `boot-stack.sh`, fail-closed if unset) — this used to be a truthiness check
on whatever the caller sent (any non-empty string "approved" a rollback); it's now a real
`hmac.compare_digest` check against that secret.
