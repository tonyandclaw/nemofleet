...
2. **送審 worker-c**(A2A `message/send` skill=`review`,metadata 帶 `{kind, subject}`;或直接 curl):
   ...
   - `kind="cve"`,subject = worker-b 定期 CVE 掃描的一筆 finding(含 `cve` / `verdict` / `component` / `our_version`)。
   - `kind="source"`,subject = worker-b 原始碼掃描(`source-scan`)`cve_reconciled` 裡 `verdict=="affected"` 的一筆(欄位同 `cve`,詳見 `it-delegate-security-scan` 技能)——走同一個審查閘(`review_cve`),因為判準一樣(元件+版本佐證、CVE id、版本一致性)。`sast_findings`(Semgrep 命中)沒有這些欄位,不送這關。
3. 讀判決 `verdict`:
   - **approve** → 放行:回報結案 / 套用。
...
5. worker-c 自己的高風險動作(`firmware-apply` / `rollback`)→ **走 `firmware-approval` 技能**(真的問人、拿到人的明確同意才簽發 `approval_token`,見該技能;不是自己編一個字串或把使用者的任意回覆當核准);沒有照那支技能拿到的 token 一律不做。
