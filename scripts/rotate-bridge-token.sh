...
# 舊 token 立即失效;policy(worker_bridge)用 IP 不含 token,故不需重渲染。
...
for entry in "worker-a:A" "worker-b:B"; do
  ...
  docker exec -u 0 "$ct" sh -c 'pkill -f worker-itops; true' >/dev/null 2>&1; sleep 1
  docker exec -d -u 0 -e BRIDGE_TOKEN="$NEW" -e BRIDGE_ZONE="$zone" "$ct" sh -c 'cd /tmp && python3 /usr/local/bin/worker-itops.py >>/tmp/worker-itops.log 2>&1'
  sleep 2
...
CT_LEAD=$(resolve_ct team-lead); OCIP=$(ocip worker-a); OC2IP=$(ocip worker-b)
if [ -n "$CT_LEAD" ] && [ -f "$SKILLS_DIR/hermes/it-delegate-worker/SKILL.md" ]; then
  SK=/sandbox/.hermes/skills/devops/it-delegate-worker/SKILL.md
  sed -e "s/172\.18\.0\.2/${OCIP:-172.18.0.2}/g" -e "s/172\.18\.0\.4/${OC2IP:-172.18.0.4}/g" -e "s/BRIDGETOKEN/$NEW/g" \
    "$SKILLS_DIR/hermes/it-delegate-worker/SKILL.md" > /tmp/it-del-rot.md
  docker cp /tmp/it-del-rot.md "$CT_LEAD:$SK" >/dev/null 2>&1 \
    && docker exec -u 0 "$CT_LEAD" sh -c "chown 998:998 $SK" >/dev/null 2>&1 \
