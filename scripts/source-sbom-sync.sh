...
# source-sbom-sync.sh — [手動 fallback] 從 gnuton/asuswrt-merlin.ng 抽「元件→版本」真實 SBOM,寫進 OpenClaw B(zone B)沙箱。
# 注:正常情況 OpenClaw B 已會自己抓(endpoint fetch_upstream_sbom,容器內經治理 egress);本腳本僅在容器無 egress 時手動補。
...
[ -n "$CTB" ] || { echo "[sbom] zone B(openclaw-2)容器未跑" >&2; exit 1; }
