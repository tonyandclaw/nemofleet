...
| **worker-c** (UI `:18793`) | Governance worker — config backup / rollback (real, per-action approval token required), **QA review gate over a/b** (binding reject → redo), SkillOS skill curation. The firmware download/apply lifecycle itself is still a stub — see `docs/design/worker-c-spec.md`. |
...
Secrets are **never** committed: TLS keys/certs, the bridge token, the worker-c approval signing
key (`services/bridge/.approval-key` — an HMAC key shared by worker-c and team-lead only, used to
mint/verify per-action, expiring, single-use `approval_token`s for rollback/firmware-apply; not
