#!/usr/bin/env python3
# openclaw-fix-endpoint.py — OpenClaw 的「真・入站端點」,跑在 my-assistant 容器內(root)。
# Hermes(經 scoped openclaw_bridge policy)POST /fix {"bug":"fw|subnet|bandwidth|dhcp|drift"} 來委派,
# 本端點植入該場景檔 → nsenter 進 gateway netns 驅動 `openclaw agent` 修 → 驗收 → 回 JSON。
# 這是唯一讓 Hermes 能驅動 OpenClaw 的入口(白名單意圖、scoped policy);非此即兩沙箱隔離。
...
# ── drift 場景素材:真實 ASUS RT-AX89X lab 設定檔(已去識別化) ──────────────
# 來源:nemoclaw-enterprise-deck/implementation/01-inventory/{baselines,current}/lab-asus-rt-ax89x-01.conf
# current 蓄意「不含」DETECTABLE DRIFT 答案註解——漂移要由 agent 自己 diff 出來。
BASELINE_CONF = """\
# Approved redacted baseline — lab-asus-rt-ax89x-01
# Source: ASUS RT-AX89X export 2026-05-15 (approved by Tony)
# Redaction policy: WAN IPs, MAC, PSK, admin passwords removed.

device.model = RT-AX89X
device.firmware = 3.0.0.4.388_24698
device.timezone = Asia/Taipei
device.admin.user = <REDACTED>
device.admin.password = <REDACTED>

wan.mode = pppoe
wan.username = <REDACTED>
wan.password = <REDACTED>

lan.ip = 192.168.50.1
lan.netmask = 255.255.255.0
lan.dhcp.enabled = true
lan.dhcp.range = 192.168.50.100-192.168.50.250

wifi.5g.ssid = LAB-ASUS-5G
wifi.5g.security = wpa3-personal
wifi.5g.psk = <REDACTED>
wifi.5g.channel = auto
wifi.5g.bandwidth = 80

wifi.6g.ssid = LAB-ASUS-6G
wifi.6g.security = wpa3-personal
wifi.6g.psk = <REDACTED>
wifi.6g.channel = auto
...
webui.http.enabled = false
webui.port.https = 443
webui.tls.cert = <REDACTED_FINGERPRINT>

firewall.wan_to_lan.default = drop
firewall.lan_to_wan.default = allow
firewall.dos_protection = enabled

logging.remote.enabled = true
logging.remote.host = syslog.lab.asus.internal
logging.remote.port = 514
logging.remote.protocol = udp
"""

DRIFTED_CONF = """\
# Current redacted evidence — lab-asus-rt-ax89x-01
# Source: ASUS RT-AX89X export 2026-05-22 (auto-collected, awaiting review)
# Redaction policy: WAN IPs, MAC, PSK, admin passwords removed.
...
logging.remote.enabled = false
logging.remote.host =
logging.remote.port =
logging.remote.protocol =
"""

# 驗收器:安全退化(必須修回 baseline)與一般漂移(留給人審)分開計
DRIFTCHECK_PY = """\
# driftcheck.py — 比對 current 與已核准 baseline:安全退化必須=0,一般漂移留給人審
import re
def kv(p):
    d = {}
    ...
    return d
W = "/sandbox/.openclaw/workspace/it-task"
base, cur = kv(f"{W}/rt-ax89x-baseline.conf"), kv(f"{W}/rt-ax89x-current.conf")
SECURITY = ["ssh.password_login", "logging.remote.enabled", "logging.remote.host",
            "logging.remote.port", "logging.remote.protocol"]
reg = [k for k in SECURITY if cur.get(k) != base.get(k)]
drift = [k for k in sorted(set(base) | set(cur)) if k not in SECURITY and cur.get(k) != base.get(k)]
print(f"REGRESSIONS={len(reg)}" + (f"({','.join(reg)})" if reg else "")
      + f" DRIFTS={len(drift)}" + (f"({','.join(drift)})" if drift else ""))
"""

# 場景表(對齊 host lib.sh bug_scenario;drift 為多檔場景,僅 bridge 路徑支援)
# 欄位:file=要修的檔 / marker=驗收子字串 / buggy 或 files=植入內容 / check=驗收指令 / oc=給 agent 的描述
SC = {
  "fw": {
    "file": "fwcheck.py", "marker": "STATUS=UPDATE_AVAILABLE",
    "buggy": ('# fwcheck.py — 路由器韌體更新檢查\n'
              '# 現役 386_59,雲端最新 386_510;應 STATUS=UPDATE_AVAILABLE\n'
              'cur, latest = "386_59", "386_510"\n'
              'status = "UP_TO_DATE" if cur >= latest else "UPDATE_AVAILABLE"  # BUG: 版本號用字串比較\n'
              'print(f"STATUS={status}")\n'),
    "oc": '是路由器韌體更新檢查腳本,有 bug:現役 386_59、雲端最新 386_510,應印 STATUS=UPDATE_AVAILABLE 卻印 UP_TO_DATE。根因是把版本號當字串比較。請把版本號 build number(底線後數字)解析成整數再比較。',
  },
  "subnet": {
    "file": "subnet.py", "marker": "HOSTS=62",
    "buggy": ('# subnet.py — 路由器 LAN /26 可用主機數;應 HOSTS=62\n'
              'prefix = 26\n'
              'hosts = 2 ** (32 - prefix)         # BUG: 漏扣網路位址與廣播位址\n'
              'print(f"HOSTS={hosts}")\n'),
    "oc": '是子網路可用主機數計算腳本,有 bug:/26 應印 HOSTS=62 卻印 64。根因是沒扣掉網路位址與廣播位址。請改成減 2。',
  },
  "bandwidth": {
    "file": "throughput.py", "marker": "SECONDS=80",
    "buggy": ('# throughput.py — 100 Mbps 傳 1000 MB 預估秒數;應 SECONDS=80.0\n'
              'mbps, size_mb = 100, 1000\n'
              'seconds = size_mb / mbps           # BUG: Mbps(bit) 當 MB/s(byte),漏除以 8\n'
              'print(f"SECONDS={seconds}")\n'),
    "oc": '是傳輸時間估算腳本,有 bug:100 Mbps 傳 1000 MB 應印 SECONDS=80.0 卻印 10.0。根因是把 Mbps(bit/s)當成 MB/s(byte/s),漏了除以 8。請把頻寬除以 8 再算。',
  },
  "dhcp": {
    "file": "dhcp.py", "marker": "POOL=101",
    "buggy": ('# dhcp.py — DHCP 位址池 .100–.200(含頭尾);應 POOL=101\n'
              'start, end = 100, 200\n'
              'pool = end - start                 # BUG: 含頭尾應 +1\n'
              'print(f"POOL={pool}")\n'),
    "oc": '是 DHCP 位址池大小計算腳本,有 bug:.100–.200 含頭尾應印 POOL=101 卻印 100。根因是 end-start 沒 +1。請改成 end-start+1。',
  },
  "drift": {
    "file": "rt-ax89x-current.conf", "marker": "REGRESSIONS=0",
    "files": {
      "rt-ax89x-baseline.conf": BASELINE_CONF,
      "rt-ax89x-current.conf": DRIFTED_CONF,
      "driftcheck.py": DRIFTCHECK_PY,
    },
    "check": f"python3 {WD}/driftcheck.py",
    "timeout": 360,
    "oc": ('是一台 ASUS RT-AX89X 路由器的「現況設定」,同目錄的 rt-ax89x-baseline.conf 是「已核准基準」。'
           '請先讀兩個檔逐鍵比對找出所有漂移,然後**只修回安全退化**:ssh.password_login 改回 false、'
           'logging.remote.enabled/host/port/protocol 全部修回 baseline 的值。'
           '其他漂移(wifi.5g.channel、wifi.5g.bandwidth、ssh.port)是待審變更,**不要動**,'
           '只在回報中列出留給人審核。'),
  },
}
...
JIRA_SEQ = {"n": 0}
# 受治理的 Jira egress(經 OpenClaw L7 proxy → policy:jira)。host.openshell.internal:3690 = host 上的 mock Jira。
JIRA_URL = os.environ.get("BRIDGE_JIRA_URL", "http://host.openshell.internal:3690/rest/api/2/issue")
JIRA_PROXY = os.environ.get("BRIDGE_JIRA_PROXY", "http://10.200.0.1:3128")

def _post_jira_governed(ticket):
    """把工單經 OpenClaw 的 L7 proxy 送到 Jira(受 policy:jira 治理 → OPA log 留 ALLOWED ... [policy:jira])。
    best-effort:送不出去不影響本機工單檔。回傳 (ok, 摘要)。"""
    gw = sh("pgrep -f openclaw-gateway | head -1").stdout.strip()
    if not gw:
        return (False, "no gateway netns")
    payload = json.dumps({"fields": {"project": {"key": "NETOPS"}, "issuetype": {"name": "Task"},
...
def open_jira(summary, description, kind, asset="lab-asus-rt-ax89x-01", priority="High"):
    """OpenClaw 的升級路徑:修不了 / 需人工核准 → 開 Jira 工單給工程師(人在迴路)。
    ...
    ticket = {"id": tid, "project": "NETOPS", "summary": summary, "kind": kind,
              ...
              "created": time.strftime("%Y-%m-%d %H:%M:%S"), "source": "OpenClaw IT operator"}
...
# ── OpenClaw 的「監控」職責:設備狀態巡檢 + 定期 CVE 掃描(確定性,零 LLM) ──────────
# 機隊 inventory(來源:enterprise-deck 真實 lab 5 台)。ASUS 韌體無完整 SBOM → 標 unknown_inventory_gap,不假裝安全。
FLEET = [
  {"asset": "lab-asus-rt-ax89x-01", "model": "RT-AX89X", "firmware": "3.0.0.4.388_24698", "sbom": False},
  {"asset": "lab-asus-xt12-mesh-01", "model": "XT12", "firmware": "3.0.0.4.388_24210", "sbom": False},
  {"asset": "lab-asus-expertwifi-eba63-01", "model": "EBA63", "firmware": "6.51.0.99", "sbom": False},
  {"asset": "lab-openwrt-gateway-01", "model": "GL-AXT1800", "firmware": "23.05.4", "sbom": True,
   "packages": {"dropbear": "2022.83-3", "openssl": "3.0.12-1", "uhttpd": "2023-06-25-34a8a74d-2"}},
  {"asset": "lab-asus-esc4000a-e12-01", "model": "ESC4000A-E12", "firmware": "BMC 2.21.00, BIOS 1701", "sbom": False},
  {"asset": "lab-asus-ebg19p-01", "model": "EBG19P", "firmware": "3.0.0.6.102_45537", "sbom": False},  # ExpertWiFi 商用 VPN 閘道(真實第三台受管設備)
]
# 06-14 職責分工:ZONE A=IT 運維/網路管理(monitor+fix,管 rt-ax89x+ebg19p),
#                 ZONE B=資安/原始碼分析(cve 掃全機隊 + source SBOM/SAST/設計文件 + 監控 openwrt)。
...
ZONE_MONITOR = {
  "A": {"lab-asus-rt-ax89x-01", "lab-asus-ebg19p-01"},   # 運維管:旗艦路由 + 商用閘道
  "B": {"lab-openwrt-gateway-01"},                        # 資安節點也監控 openwrt
}
ZONE_CAPS = {"A": {"monitor", "fix", "cert"}, "B": {"monitor", "cve", "source"}}
def _zone_has(cap):
...
def run_cve_scan(trigger="api"):
    ...
    # 真實 SBOM:有原始碼 SBOM 的資產(rt-ax89x)設備 CVE 分級也用真版本,與原始碼節點一致(不再 unknown)
    ...
    # 即時 CVE 情資(NVD):對有真實 SBOM 的資產(rt-ax89x)用版本即時查 NVD,補上固定清單沒有的當下 CVE
    try:
...
def _cve_schedule_loop():
    """OpenClaw 監控職責的「定期」本體:端點起來後先掃一輪(boot 即巡檢),之後每 CVE_INTERVAL 秒掃一次。
    ...
    while True:
        ...
        if iv and iv > 0:
            try:
                # OpenClaw B 自主:先抽 upstream SBOM/原始碼(SAST),產出真實 source-sbom.json 供設備 CVE 分級用真版本。
                # zone A 會在函式內早退(非其職責);fetch 自帶 12h 快取,不會每輪打 github。
                run_source_scan()
            except Exception as e:
                print(f"[SOURCE SCHEDULE] {e}", flush=True)
            try:
                run_cve_scan(trigger="schedule")
            except Exception as e:
                print(f"[CVE SCHEDULE] {e}", flush=True)
            try:
                run_syslog_analysis()   # OpenClaw A:syslog 異常/根因/融合分析(zone B 無 syslog 檔→no-op)
            except Exception as e:
  ...
  "lab-asus-rt-ax89x-01": {
    "certs": [
      {"service": "Web UI (HTTPS:8443)", "cn": "router.asus.com", "issuer": "self-signed", "self_signed": True,
       "sig_alg": "sha256WithRSAEncryption", "key_type": "RSA", "key_bits": 2048, "not_after": "2026-06-22"},
    ],
    "tls": [{"service": "Web UI (HTTPS:8443)", "versions": ["TLSv1.1", "TLSv1.2", "TLSv1.3"],
             "ciphers": ["ECDHE-RSA-AES256-GCM-SHA384", "AES128-SHA"]}],
    "ssh": [],
  },
}
...
def _cert_schedule_loop():
    ...
    while True:
        ...
        if iv and iv > 0:
            try:
                run_cert_scan(trigger="schedule")
            except Exception as e:
                print(f"[CERT SCHEDULE] {e}", flush=True)
            time.sleep(max(int(iv), 300))
        else:
            time.sleep(600)
# 安全鍵清單(與 DRIFTCHECK_PY 內的 SECURITY 一致):這些鍵偏離 baseline = 安全退化,必須告警
SECURITY_KEYS = ["ssh.password_login", "logging.remote.enabled", "logging.remote.host",
                 "logging.remote.port", "logging.remote.protocol"]
def _conf_kv(path):
    d = {}
    try:
        for line in open(path, encoding="utf-8"):
            m = _re_pending.match(r"\s*([A-Za-z0-9_.]+)\s*=\s*(.*?)\s*$", line)  # 含大寫:支援 OpenWrt UCI 鍵
            if m:
...
# 第二台機隊資產:OpenWrt GL-AXT1800(真實 lab 素材,UCI export)。讓「機隊 N 台」字面成立,不只一台 sample。
# 預設 current=baseline(設備健康);CVE 面已由 FLEET 涵蓋(有 SBOM → dropbear/openssl affected)。
OPENWRT_BASELINE = """\
# Approved redacted baseline — lab-openwrt-gateway-01 (UCI export)
# Source: OpenWrt GL-AXT1800 23.05.4 export 2026-05-14 (approved by Tony)
# Redaction policy: WAN IPs, MAC, keys removed.

system.hostname = lab-openwrt-gw
system.openwrt_version = 23.05.4

dropbear.PasswordAuth = off
dropbear.RootPasswordAuth = off
dropbear.Port = 22

uhttpd.listen_http =
uhttpd.listen_https = 192.168.1.1:443
uhttpd.redirect_https = on

firewall.wan_to_lan.default = REJECT
firewall.rule.luci_wan = absent

dnsmasq.dnssec = on
dnsmasq.rebind_protection = on

pkg.dropbear = 2022.83-3
pkg.openssl = 3.0.12-1
"""
# OpenWrt 安全鍵:偏離 baseline = 安全退化(SSH 密碼登入、明文 admin listener、LuCI 暴露 WAN、DNSSEC 關)
OPENWRT_SECURITY = ["dropbear.PasswordAuth", "uhttpd.listen_http", "firewall.rule.luci_wan", "dnsmasq.dnssec"]
...
EBG19P_BASELINE = """\
# Approved redacted baseline — lab-asus-ebg19p-01 (ASUS ExpertWiFi)
# Source: ASUS ExpertWiFi EBG19P firmware 3.0.0.6.102_45537 export 2026-06-13 (approved by Tony)
# Redaction policy: WAN IPs, VPN keys, admin passwords removed.

device.model = EBG19P
device.firmware = 3.0.0.6.102_45537
device.timezone = Asia/Taipei

webui.https.enabled = true
webui.http.enabled = false
webui.wan_access = false
webui.port.https = 8443
...
vpn.server.enabled = true
vpn.server.type = wireguard
vpn.server.cipher = chacha20poly1305

sdn.guest.isolation = enabled
vlan.management.id = 1

poe.budget.watts = 123
poe.ports.poweron = 8
...
EBG19P_SECURITY = ["ssh.password_login", "ssh.wan_access", "webui.http.enabled", "webui.wan_access",
                   "firewall.wan_to_lan.default", "firewall.dos_protection",
                   "upnp.enabled", "wps.enabled"]  # 2026-06-16 UPnP/WPS/DoS 硬合規;遠端 syslog 經使用者核准接受現況→移出(不告警)

# 受監控機隊清單(每台:設定檔 + 自己的安全鍵 + 巡檢顯示欄位)。OpenClaw 逐台比對核准基準。
MANAGED = [
  {"asset": "lab-asus-rt-ax89x-01", "baseline": "rt-ax89x-baseline.conf", "current": "rt-ax89x-current.conf", "live": True,
   "security": SECURITY_KEYS, "show": ["ssh.password_login", "logging.remote.enabled", "ssh.port"]},
  {"asset": "lab-openwrt-gateway-01", "baseline": "openwrt-baseline.conf", "current": "openwrt-current.conf",
   "security": OPENWRT_SECURITY, "show": ["dropbear.PasswordAuth", "uhttpd.listen_https", "dnsmasq.dnssec"]},
...
def seed_monitor_assets():
    """確保非 fix 場景的受管設備設定檔存在(OpenWrt 第二台,預設 current=baseline=健康)。冪等。"""
    os.makedirs(WD, exist_ok=True)
    wrote = False
    seeds = {"openwrt-baseline.conf": OPENWRT_BASELINE, "openwrt-current.conf": OPENWRT_BASELINE,
             "ebg19p-baseline.conf": EBG19P_BASELINE, "ebg19p-current.conf": EBG19P_BASELINE,
             "rt-ax89x-baseline.conf": BASELINE_CONF, "rt-ax89x-current.conf": BASELINE_CONF}
...
def run_syslog_analysis():
    """OpenClaw A 自主:EBG19P syslog 進階分析 — ① 異常偵測(auth 暴力/wifi 洪水/firewall/kernel)
...
def monitor_scan():
    ...
    for dev in rep.get("devices", []):
        ...
        desc = ("OpenClaw 定期合規巡檢:現況偏離已核准安全基準的鍵 — "
...
# 對「現在拿得到原始碼」的這台 ASUS 設備,植入一小段韌體 source(含已知弱點樣式)讓 OpenClaw 讀。
SRC_DIR = f"{WD}/source"
...
# B8:每個已知弱點檔的「建議修正版」——OpenClaw 據此產 unified diff 附進 Jira 給工程師審
SRC_FIXES = {
  "diag.c": ("/* diag.c — LuCI 網路診斷:ping 工具(節錄) */\n"
             "#include <stdlib.h>\n#include <stdio.h>\n#include <unistd.h>\n#include <ctype.h>\n#include <string.h>\n\n"
...
# ── OpenClaw B 自主上游分析:從韌體 repo 抽 SBOM + 真實含 sink 的原始碼(治理 egress;結果快取於 WD)──
...
def _gh_json(url):
    req = _urlreq.Request(url, headers={"User-Agent": "openclaw-b-secanalysis", "Accept": "application/vnd.github+json"})
...
def _gh_raw(path):
    req = _urlreq.Request(f"https://raw.githubusercontent.com/{SRC_REPO}/{SRC_REF}/{path}",
                          headers={"User-Agent": "openclaw-b-secanalysis"})
...
def fetch_upstream_sbom():
    """OpenClaw B 自主:列 upstream release/src/router → 元件→版本(同元件取最高)→ 寫 source-sbom.json。"""
    ...
    for it in items:
        ...
        if n not in pk or _vt(v) > _vt(pk[n]):
            pk[n] = v
    data = {"ts": time.strftime("%Y-%m-%d %H:%M:%S"), "source": "github (OpenClaw B autonomous)",
...
def fetch_upstream_sast():
    """OpenClaw B 自主:從 upstream 抓真實含 sink 的 C 檔 → base/real/ + manifest(供 SAST 掃真 repo 碼)。"""
    ...
    return data
def _load_real_sbom():
    """設備 CVE 分級用:把 OpenClaw B 抽到的真實 SBOM 對應到資產(目前 SRC_ASSET=rt-ax89x)。"""
    out = {}
    try:
...
def fetch_upstream_advisories():
    """OpenClaw B 自主:讀 upstream 真實 Changelog-NG.txt → 已修補(FIXED/resolved)CVE 清單 + 證據行。
...
def _reconcile_advisories(findings, fixed):
    ...
    return n
# ── OpenClaw B 自主:即時 CVE 情資(OSV.dev)── 取代固定 CVE 清單,讓每日掃隨上游新公布 CVE 變動 ──
...
def _nvd_query(prod, ver):
    ...
    hdr = {"User-Agent": "openclaw-b-secanalysis"}
    if NVD_API_KEY:
    ...
    for _ in range(2):
        try:
            req = _urlreq.Request("https://api.osv.dev/v1/query", data=data,
                                  headers={"Content-Type": "application/json", "User-Agent": "openclaw-b-secanalysis"})
...
def refresh_live_cves(pkgs):
    ...
    try:
        ...
        for comp in LIVE_COMPONENTS:
            ...
            try:
                json.dump({"ts": time.strftime("%Y-%m-%d %H:%M:%S"), "source": f"{src} (OpenClaw B live)",
...
def run_source_scan():
    ...
    # OpenClaw B 自主:掃描前自己經 egress 從 upstream 韌體 repo 抽 SBOM + 真實含 sink 的原始碼(12h 快取)。
    ...
    # 真實原始碼:OpenClaw B 抓到的含 sink C 檔放在 base/real/(附 manifest)。
    ...
    # OpenClaw B 自主:即時 CVE 情資(NVD)— 用 SBOM 版本查當下適用的 CVE,不再只靠固定清單(每日掃會隨上游新公布變動)
    live_src = "—"
    try:
    ...
    # OpenClaw B 自主:讀真實上游 changelog,把因 backport 已修的 affected 校正為 not_affected(避免版本式假陽性)
    adv = {}
    try:
    ...
    # config 類對照 rt-ax89x-current.conf;code 類對照 SAST 命中(發現回標違反的設計條款,進 Jira 引用)。
    design, cur = [], _conf_kv(f"{WD}/rt-ax89x-current.conf")
    ...
    report = {"ts": time.strftime("%Y-%m-%d %H:%M:%S"), "asset": SRC_ASSET,
              "analysis_by": "OpenClaw B · 自主上游抓取(治理 egress)", "upstream_repo": f"{SRC_REPO}@{SRC_REF}",
...
def run_fix(bug):
    ...
    path = f"{WD}/{sc['file']}"
    os.makedirs(WD, exist_ok=True)
    for name, content in (sc.get("files") or {sc["file"]: sc["buggy"]}).items():
        with open(f"{WD}/{name}", "w") as f:
            f.write(content)
    sh(f"chown -R 998:998 {WD}")
    check = sc.get("check") or f"python3 {path}"
    before = sh(check).stdout.strip()
    gw = sh("pgrep -f openclaw-gateway | head -1").stdout.strip()
    if not gw:
        return {"ok": False, "error": "openclaw-gateway 未在跑", "bug": bug}
    msg = (f"你是管理 ASUS 網路產品的 IT operator。{path} {sc['oc']} "
           f"請用 read 工具讀檔、用 write 工具寫回修正版,只用 read/write 不要 exec。修好回報你改了什麼。")
    sid = f"bridge-{int(time.time())}"
    t = sc.get("timeout", 240)
    # nsenter 進 gateway netns 跑 openclaw agent(本端點已在容器內、root)
    agent = subprocess.run(
        ["nsenter", "-t", gw, "-n", "sh", "-lc",
         f'HOME=/sandbox openclaw agent --agent main --session-id {sid} '
         f'-m "{msg}" --thinking low --timeout {t}'],
        capture_output=True, text=True, timeout=t + 60)
    raw = agent.stdout + agent.stderr
    clean = re.sub(r"\x1b\[[0-9;]*m", "", raw)
    tail = "\n".join([l for l in clean.splitlines()
                      if l.strip() and not re.search(r"staging bundled|failed to stage|Config warnings|plugin|npm error|stale config", l)][-4:])
    after = sh(check).stdout.strip()
    res = {"ok": sc["marker"] in after, "bug": bug, "file": sc["file"],
           "before": before, "after": after, "marker": sc["marker"], "agent_tail": tail,
           "secs": round(time.time() - _t0, 1), "ts": time.strftime("%Y-%m-%d %H:%M:%S")}
    print(f"[FIX DONE] bug={bug} before={before} -> after={after} ok={res['ok']}", flush=True)
    # 升級路徑(thesis:修不了 / 需人工核准 → OpenClaw 開 Jira 工單請工程師)
    if not res["ok"]:
        # 自動修不好 → 升級工程師
        res["jira"] = open_jira(
            f"OpenClaw 無法自動修復 {sc['file']}({bug}),需工程師介入",
            f"自動修復後驗收未通過。預期 marker={sc['marker']};實際 before={before} -> after={after}。"
            f"agent 摘要:{tail}", kind="auto-fix-failed", priority="High")
        res["escalated"] = True
    elif bug == "drift":
        # 安全退化已自動修回;一般漂移屬「需人工核准的變更」→ 開單留審(人在迴路)
        pend = _pending_drifts(after)
        if pend:
            res["jira"] = open_jira(
                f"RT-AX89X 設定變更待人工核准({len(pend)} 項)",
                "OpenClaw 已自動修回所有安全偏離(REGRESSIONS=0)。下列變更非安全偏離、屬需人工判斷的調整,"
                "已保留現狀未動,開單請工程師審核:\n- " + "\n- ".join(pend),
                kind="change-approval", priority="Medium")
            res["pending_review"] = pend
    return res

# ── 真實 EBG19P 設定 remediation(OpenClaw A 維運動作;device-aware,回報正確設備)──
# cred 由 boot-stack 以 -e EBG19P_CRED="ip|user|pass" 注入(僅 zone A;A 管 ebg19p)。容器可直連 EBG。
EBG_CRED = os.environ.get("EBG19P_CRED", "").strip()
EBG_ACTIONS = {   # bug → (nvram key, 目標值, action_script, 人類描述) — OpenClaw A 對 EBG19P 的確定性安全操作集
  # 無線
...
def _ebg_login(ip, user, pw, jar):
    import urllib.parse as _up
    cred = __import__("base64").b64encode(f"{user}:{pw}".encode()).decode()
    body = _up.urlencode({"login_authorization": cred}).encode()
    req = _urlreq.Request(f"http://{ip}/login.cgi", data=body,
                          headers={"Content-Type": "application/x-www-form-urlencoded", "Referer": f"http://{ip}/Main_Login.asp"})
    try:
        resp = _urlreq.urlopen(req, timeout=12)
        ...
        pass
    return None
def _ebg_nvget(ip, tok, key):
    req = _urlreq.Request(f"http://{ip}/appGet.cgi?hook=nvram_get({key})",
                          headers={"Cookie": f"asus_token={tok}", "Referer": f"http://{ip}/index.asp"})
    try:
        return str(list(json.load(_urlreq.urlopen(req, timeout=8)).values())[0])
    except Exception:
        return None
def run_ebg_remediate(bug):
    ...
    if not EBG_CRED or EBG_CRED.count("|") < 2:
        res = {"ok": False, "bug": bug, "asset": asset, "error": "EBG19P 憑證未注入(boot-stack 需傳 EBG19P_CRED)",
               "ts": time.strftime("%Y-%m-%d %H:%M:%S")}
        return res
    ip, user, pw = EBG_CRED.split("|", 2)
    tok = _ebg_login(ip, user, pw, None)
    if not tok:
        return {"ok": False, "bug": bug, "asset": asset, "error": f"EBG19P 登入失敗({ip})", "ts": time.strftime("%Y-%m-%d %H:%M:%S")}
    before = _ebg_nvget(ip, tok, key)
    # applyapp.cgi:寫 nvram(可多鍵)+ 套用
    _payload = {"action_mode": "apply", "action_script": script, "action_wait": "10"}
    _payload.update(dict(sets))
    body = _up.urlencode(_payload).encode()
    try:
        _urlreq.urlopen(_urlreq.Request(f"http://{ip}/applyapp.cgi", data=body,
                        headers={"Cookie": f"asus_token={tok}", "Content-Type": "application/x-www-form-urlencoded",
                                 "Referer": f"http://{ip}/index.asp"}), timeout=20)
    except Exception:
        pass   # restart 會中斷連線,屬正常;以重讀為準
    # 驗證:host streamer 每 ~5s 登入 EBG 搶單一 session 會踢掉本 token → 每次讀前重登、縮短窗口、多試。
    # 讀到 want 即成功;只讀到舊值=真失敗;完全讀不到(None)=驗證被 session 佔用打斷(非修復失敗,不亂開單)。
    time.sleep(8)   # 等 restart 子系統起來
    after = None
    for _ in range(18):
        tok = _ebg_login(ip, user, pw, None) or tok   # 每次讀前重登,贏過 streamer 的搶佔
        v = _ebg_nvget(ip, tok, key)
    ...
    if verify == "changed_wrong":   # 只在「確實讀到非目標值」才升級;讀不到不開單(避免假陰性洗單)
        res["jira"] = open_jira(f"OpenClaw 無法自動套用 EBG19P {desc}",
...
def run_fix_bg(bug):
    BUSY["on"] = True
    try:
        LAST.clear(); LAST.update(run_fix(bug))
    except Exception as e:
        LAST.clear(); LAST.update({"ok": False, "bug": bug, "error": str(e)})
        print(f"[FIX ERROR] bug={bug} {e}", flush=True)
    finally:
        BUSY["on"] = False
        save_last()
        if LAST.get("secs") is not None:
            try:
                with open(FIX_HISTORY, "a", encoding="utf-8") as fh:
                    fh.write(json.dumps({"ts": LAST.get("ts"), "bug": LAST.get("bug"),
                                         "ok": LAST.get("ok"), "secs": LAST.get("secs")}, ensure_ascii=False) + "\n")
                sh(f"chown 998:998 {FIX_HISTORY}")
            except Exception:
                pass
...
class H(BaseHTTPRequestHandler):
    ...
    def _authed(self):
        ...
        # 註:本服務綁 0.0.0.0 是「跨 agent 委派(Hermes→OpenClaw :9099/fix)」必要;
        #     真正的存取控制 = OpenShell openclaw_bridge /32 政策 + 此 token 雙鎖。
        tok = self.headers.get("X-Bridge-Token", "")
        return bool(TOKEN) and hmac.compare_digest(tok, TOKEN)
    def do_GET(self):
        if self.path == "/health":
            self._send(200, {"status": "ok", "service": "openclaw-fix-endpoint", "scenarios": list(SC),
                             ...
                             "caps": sorted(ZONE_CAPS.get(ZONE, [])), "periodic_cve_sec": CVE_INTERVAL})
        ...
        elif self.path == "/log-analysis":  # EBG19P syslog 進階分析(異常/根因/融合/日報;OpenClaw A)
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, run_syslog_analysis())
        elif self.path == "/traffic":  # EBG19P WAN 流量基線 + 突增異常(node A 運維)
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, load_traffic())
        elif self.path == "/cve":    # 定期 CVE 掃描(監控職責;確定性、零 Azure)
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, run_cve_scan())
        elif self.path == "/monitor":  # 設備狀態巡檢(監控職責)
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, run_monitor())
        elif self.path == "/source-cve":  # 有原始碼:SBOM + code 證據 CVE + SAST
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, run_source_scan())
        elif self.path == "/cert-scan":  # 憑證 / 弱加密與協定盤點(運維節點 A;主動提醒)
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            self._send(200, run_cert_scan())
    ...
    def do_POST(self):
        if self.path == "/monitor-scan":   # 定期合規巡檢 + 對安全退化開 Jira(治理 egress);排程呼叫
            if not self._authed():
                return self._send(403, {"error": "X-Bridge-Token required"})
            return self._send(200, monitor_scan())
        ...
        if bug in EBG_ACTIONS or bug in EBG_MULTI:
            ...
            threading.Thread(target=run_ebg_remediate_bg, args=(bug,), daemon=True).start()
            return self._send(202, {"accepted": True, "bug": bug, "asset": "lab-asus-ebg19p-01",
                                    "note": f"OpenClaw A 已接手對 EBG19P 套用 {_desc}(約 30-60s)。完成後 GET /last。"})
        # 未知 bug 不再靜默當成 demo 'fw'(否則會對錯設備謊報成功);只接受已知情境
        if bug not in SC:
            return self._send(400, {"accepted": False, "error": f"未知修復類型 '{bug}'。支援:{list(SC) + list(EBG_ACTIONS) + list(EBG_MULTI)}"})
        # 非同步:立刻回 ack,修復在背景跑(openclaw agent turn 約 30-120s),結果查 GET /last
        threading.Thread(target=run_fix_bg, args=(bug,), daemon=True).start()
        self._send(202, {"accepted": True, "bug": bug,
                         "note": f"OpenClaw 已接手修復 {SC[bug]['file']}(約 30-120s)。完成後 GET /last 取結果,或看桌面端點 log。"})
...
if __name__ == "__main__":
    ...
    print(f"[openclaw-fix-endpoint] listening on 0.0.0.0:{PORT} "
          f"(scenarios: {list(SC)}, auth: {'on' if TOKEN else 'off'}, "
