...
def do_sys(do, sb="", tail="200", provider="", model=""):
...
document.addEventListener('click',e=>{
 ...
 else if(act==='sysdo'){const dd=b.dataset.do,sb=b.dataset.sb||'';if(b.dataset.confirm&&!confirm(b.dataset.confirm))return;const old=b.textContent;b.disabled=true;b.textContent=t('running');sysShow(dd+(sb?(' · '+sb):''),t('running')+'…');fetch('/api/sys?do='+encodeURIComponent(dd)+(sb?('&sb='+encodeURIComponent(sb)):''),{cache:'no-store'}).then(r=>r.json()).then(r=>{sysShow(r.title||dd,r.out||'(empty)');tick&&tick()}).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;b.textContent=old})}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/sys":   # on-demand;admin:doctor / logs / stale / gsettings / recover / gwhealth / infset
            ...
            provider = q.get("provider", [""])[0]; model = q.get("model", [""])[0]
            try:
                return self._send(200, json.dumps(do_sys(do, sb, tail, provider, model), ensure_ascii=False), "application/json; charset=utf-8")
            except Exception as e:
