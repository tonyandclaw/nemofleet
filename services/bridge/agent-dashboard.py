...
.clickable{cursor:pointer}.clickable:hover{background:var(--card2)}.laev{margin-top:7px;padding:7px 9px;background:var(--card2);border-radius:8px;font-size:11px;line-height:1.7;font-family:"SF Mono",ui-monospace,monospace;color:var(--tx2);word-break:break-all}
...
function vFleet(d){const a=agg(d);let cards='';
 for(const n of [a.nA,a.nB]){if(!n.label)continue;if(CFG.node!=='all'&&n.label!==CFG.node)continue;
  ...
  if(n.loganalysis){const LA=n.loganalysis,fs=(LA.findings||[]).concat(LA.fusion||[]),rc=LA.root_causes||[];
   ...
   const frows=fs.map(x=>{const ev=x.evidence||[];const xp=ev.length&&LAEXP.has(x.id);return `<div class="kv${ev.length?' clickable':''}" ${ev.length?`data-act="laexp" data-id="${esc(x.id||'')}"`:''} style="align-items:flex-start"><span class="k" style="flex:0 0 auto;color:${sc(x.sev)}">${x.sev==='high'?'⛔':'⚠︎'}</span><span class="v" style="text-align:left;flex:1">${esc(L(x,'title'))}${ev.length?` <span class="mut">${xp?'▾':'▸'}</span>`:''}${(L(x,'detail'))?`<div class="mut" style="font-size:11px;margin-top:1px">${esc(L(x,'detail'))}</div>`:''}${xp?`<div class="laev"><div class="mut" style="font-size:10.5px;margin-bottom:3px">${t('la_evidence')}</div>${ev.map(e=>`<div>${esc(e)}</div>`).join('')}</div>`:''}</span></div>`;}).join('')||`<div class="mut">${t('la_clean')}</div>`;
...
document.addEventListener('click',e=>{
 ...
 if(act==='theme'){CFG.theme=v;localStorage.setItem('nclaw-theme',v);document.documentElement.setAttribute('data-theme',v);loadColors();if(LAST)render(LAST)}
  ...
  else if(act==='cipherinfo'){CIPHERINFO=!CIPHERINFO;if(LAST)render(LAST)}
  else if(act==='laexp'){const id=b.dataset.id||'';if(LAEXP.has(id))LAEXP.delete(id);else LAEXP.add(id);if(LAST)render(LAST)}
 else if(act==='certpol'){const sc=b.dataset.scope||'',k=b.dataset.k;if(LAST&&LAST.settings){const S=LAST.settings,nv=(v===''?'':(isNaN(+v)?v:+v)),ok=(k.indexOf('dev_')===0?'dev_overrides':'cert_overrides');if(sc){S[ok]=Object.assign({},S[ok]);S[ok][sc]=Object.assign({},S[ok][sc]);if(v==='')delete S[ok][sc][k];else S[ok][sc][k]=nv;}else if(v!=='')S[k]=nv;render(LAST);}b.disabled=true;fetch('/api/cert-policy?scope='+encodeURIComponent(sc)+'&key='+encodeURIComponent(k)+'&value='+encodeURIComponent(v),{method:'POST'}).then(r=>r.json()).then(r=>toast(r.msg||t('toast_done'))).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;tick()})}
 else if(act==='certfam'){const on=b.classList.contains('on')?0:1;b.classList.toggle('on');b.disabled=true;fetch('/api/cert-policy?fam='+encodeURIComponent(v)+'&on='+on,{method:'POST'}).then(r=>r.json()).then(r=>toast(r.msg||t('toast_done'))).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;tick()})}
 ...
 else if(act==='cfg'){const k=b.dataset.k;if(LAST&&LAST.settings){LAST.settings[k]=(isNaN(+v)?v:+v);render(LAST);}b.disabled=true;b.textContent=t('running');fetch('/api/config?k='+encodeURIComponent(k)+'&v='+encodeURIComponent(v),{method:'POST'}).then(r=>r.json()).then(r=>toast(r.msg||t('toast_done'))).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;tick()})}
 else if(act==='recadd'){const n=(el('rc_name')||{}).value||'',tg=(el('rc_tg')||{}).value||'',em=(el('rc_email')||{}).value||'';if(!n.trim()){toast(t('rec_need_name'));return}if(LAST&&LAST.settings){LAST.settings.recipients=(LAST.settings.recipients||[]).concat([{name:n,telegram:tg,email:em}]);render(LAST);}b.disabled=true;fetch('/api/recipient?op=add&name='+encodeURIComponent(n)+'&telegram='+encodeURIComponent(tg)+'&email='+encodeURIComponent(em),{method:'POST'}).then(r=>r.json()).then(r=>toast(r.msg||t('toast_done'))).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;tick()})}
