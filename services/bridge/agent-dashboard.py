    ...
    nodes = [
        ...
        {"label": "B", "name": "worker-b", "zone": "zone B", "role": "資安 / 原始碼分析", "role_en": "security / source", "caps": ["cve", "nuclei"], "alive": True, "up": True, "tag": "sec", "port": 18792, "alerts": 1, "monitor": [],
         ...
         "source": {"sast_engine": "semgrep", "sast_source": "RMerl/asuswrt-merlin.ng @ main", "sbom": 312, "sbom_source": "RMerl/asuswrt-merlin.ng @ main · multi-ecosystem parse", "sast_triaged": 3, "nemotron_reviewed_files": 4, "design": [],
                    ...
                    # real files on RMerl/asuswrt-merlin.ng@main — permalinks resolve; each finding carries
                    # the full drawer detail (rule, matched code, suggested patch diff, Nemotron triage, CWE remediation)
                    "sast_list": [
                        {"cwe": "CWE-798 hardcoded-credentials", "file": "web.c", "upstream_path": "release/src/router/httpd/web.c", "line": 1487,
                         "engine": "semgrep", "check_id": "c.lang.security.hardcoded-credential-compare", "severity": "ERROR", "patch_verified": True,
                         "message": "A password is compared against a string literal baked into the binary. Anyone with the firmware image can extract it — it is not a secret.",
                         "code": "if (!strcmp(passwd, \"admin1234\")) {\n    login_ok = 1;\n}",
                         "patch": "@@ -1487,3 +1487,4 @@\n-    if (!strcmp(passwd, \"admin1234\")) {\n+    const char *ref = nvram_safe_get(\"http_passwd\");\n+    if (ref[0] && !strcmp(passwd, ref)) {\n         login_ok = 1;\n     }",
                         "triage": {"verdict": "confirmed", "confidence": 96, "by": "nemotron",
                                    "why": "Reachable from the unauthenticated login handler; the literal is a real credential, not a placeholder."},
                         "remediation": {"risk": "Credential is extractable from firmware → universal backdoor login.",
                                         "fix": "Compare against a value read from nvram/secure storage; never store credentials in source.",
                                         "ref": "https://cwe.mitre.org/data/definitions/798.html"},
                         "url": "https://github.com/RMerl/asuswrt-merlin.ng/blob/main/release/src/router/httpd/web.c#L1487"},
                        {"cwe": "CWE-78 os-command-injection", "file": "firewall.c", "upstream_path": "release/src/router/rc/firewall.c", "line": 910,
                         "engine": "semgrep", "check_id": "c.lang.security.tainted-system-command", "severity": "ERROR", "patch_verified": True,
                         "message": "An nvram value flows unsanitized into system(). A crafted value breaks out of the intended command.",
                         "code": "sprintf(cmd, \"iptables -A INPUT -s %s -j DROP\",\n        nvram_safe_get(\"block_ip\"));\nsystem(cmd);",
                         "patch": "@@ -910,3 +910,3 @@\n-sprintf(cmd, \"iptables -A INPUT -s %s -j DROP\", nvram_safe_get(\"block_ip\"));\n-system(cmd);\n+char *argv[] = {\"iptables\",\"-A\",\"INPUT\",\"-s\", nvram_safe_get(\"block_ip\"), \"-j\",\"DROP\", NULL};\n+_eval(argv, NULL, 0, NULL);",
                         "triage": {"verdict": "confirmed", "confidence": 92, "by": "nemotron",
                                    "why": "block_ip is settable via the web UI; no character validation before the shell call."},
                         "remediation": {"risk": "Remote command execution as root via a router config field.",
                                         "fix": "Never build shell strings from input — use _eval()/execv with an argument array, or strictly validate the value.",
                                         "ref": "https://cwe.mitre.org/data/definitions/78.html"},
                         "url": "https://github.com/RMerl/asuswrt-merlin.ng/blob/main/release/src/router/rc/firewall.c#L910"},
                        {"cwe": "CWE-120 unbounded-copy", "file": "cgi.c", "upstream_path": "release/src/router/httpd/cgi.c", "line": 212,
                         "engine": "semgrep", "check_id": "c.lang.security.strcpy-into-fixed-buffer", "severity": "WARNING",
                         "message": "strcpy() copies a request-controlled string into a 128-byte stack buffer with no bound.",
                         "code": "char host[128];\nstrcpy(host, webcgi_get(\"hostname\"));",
                         "patch": "@@ -212,2 +212,2 @@\n-char host[128];\n-strcpy(host, webcgi_get(\"hostname\"));\n+char host[128];\n+strlcpy(host, webcgi_get(\"hostname\"), sizeof(host));",
                         "triage": {"verdict": "likely", "confidence": 74, "by": "nemotron",
                                    "why": "Overflow depends on whether an upstream length cap exists on hostname — needs human confirmation."},
                         "remediation": {"risk": "Stack buffer overflow → possible RCE / crash from a long field.",
                                         "fix": "Use strlcpy/snprintf with sizeof(dst); bound every copy of external input.",
                                         "ref": "https://cwe.mitre.org/data/definitions/120.html"},
                         "url": "https://github.com/RMerl/asuswrt-merlin.ng/blob/main/release/src/router/httpd/cgi.c#L212"},
                        {"cwe": "CWE-134 format-string", "file": "httpd.c", "upstream_path": "release/src/router/httpd/httpd.c", "line": 560,
                         "engine": "nemotron",
                         "message": "The request User-Agent header is passed as the format argument to syslog() — a classic format-string sink.",
                         "code": "syslog(LOG_INFO, user_agent);",
                         "patch": "@@ -560 +560 @@\n-syslog(LOG_INFO, user_agent);\n+syslog(LOG_INFO, \"%s\", user_agent);",
                         "triage": {"verdict": "likely", "confidence": 80, "by": "nemotron",
                                    "why": "No Semgrep ruleset for this pattern in C here — flagged by Nemotron review; %n write-primitive plausible."},
                         "remediation": {"risk": "Attacker-controlled format specifiers → info leak or memory write.",
                                         "fix": "Always use a fixed format string: syslog(LOG_INFO, \"%s\", data).",
                                         "ref": "https://cwe.mitre.org/data/definitions/134.html"},
                         "url": "https://github.com/RMerl/asuswrt-merlin.ng/blob/main/release/src/router/httpd/httpd.c#L560"},
                        {"cwe": "CWE-676 dangerous-function", "file": "init.c", "upstream_path": "release/src/router/rc/init.c", "line": 305,
                         "engine": "semgrep", "check_id": "c.lang.security.raw-system-call", "severity": "WARNING", "violates_design": "no-raw-system",
                         "message": "Raw system() call — violates the approved 'no-raw-system' baseline (all shell-outs must go through _eval).",
                         "code": "system(nvram_safe_get(\"script_usbmount\"));",
                         "patch": "@@ -305 +305,2 @@\n-system(nvram_safe_get(\"script_usbmount\"));\n+char *argv[] = {\"/bin/sh\",\"-c\", nvram_safe_get(\"script_usbmount\"), NULL};\n+_eval(argv, NULL, 0, NULL);",
                         "triage": {"verdict": "confirmed", "confidence": 88, "by": "nemotron",
                                    "why": "Matches the design-baseline rule; script_usbmount is user-settable."},
                         "remediation": {"risk": "Shell-out of a user-controlled script path — command execution surface.",
                                         "fix": "Route through _eval() with an argv array (project convention); drop raw system().",
                                         "ref": "https://cwe.mitre.org/data/definitions/676.html"},
                         "url": "https://github.com/RMerl/asuswrt-merlin.ng/blob/main/release/src/router/rc/init.c#L305"}]}},
...
ALLOWED_CFG = {"cve_interval_sec", "cert_interval_sec", "cert_expire_warn_days", "cert_rsa_min",
               ...
               "nuclei_interval_sec", "nuclei_tags", "nuclei_targets", "proactive_snooze_until", "backup_interval_sec",
