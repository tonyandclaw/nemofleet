...
DIR = os.environ.get("NEMOFLEET_ROOT") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # repo root
...
ALLOWED_CFG = {"cve_interval_sec", "cert_interval_sec", "cert_expire_warn_days", "cert_rsa_min",
               "auto_escalate", "quiet_enabled", "quiet_start", "quiet_end", "quiet_days", "notify_channels", "cert_sig_min", "cert_cipher_policy",
...
def do_config(k, v):
    ...
    for frag in ("my-assistant", "openclaw-2"):
        ...
        if '"ok":true' in out.replace(" ", ""):
            ok = True
    if ok and k in ("cert_rsa_min", "cert_expire_warn_days", "cert_sig_min", "cert_cipher_policy"):  # 改門檻 → 立刻重掃刷新報表(避免時間差)
        cta = ct("my-assistant")
        if cta:
...
def do_sys(do, sb="", tail="200"):
    # 中價值:較重的診斷,改 on-demand(按鈕觸發,不進 5s 輪詢)
    if do == "doctor":
...
/* settings */
.setrow{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:13px 0;border-top:1px solid var(--line)}
...
function vSettings(d){
 ...
 const mgmtThresh=`<div class="card"><div class="ct"><span class="ico" style="background:var(--card2);color:var(--warn)">🎯</span><b>${t('set_mgmt_thresh')}</b><span style="margin-left:auto">${scopeSeg}</span></div>${srow(t('set_warn_days'),t('set_warn_days_d'),certsel('cert_expire_warn_days',[['7','7d'],['14','14d'],['30','30d'],['60','60d'],['90','90d']]))}${srow(t('set_rsa_min'),t('set_rsa_min_d'),certsel('cert_rsa_min',[['2048','2048'],['3072','3072'],['4096','4096']]))}${srow(t('set_sig'),t('set_sig_d'),certsel('cert_sig_min',[['sha1','SHA-1'],['sha256','SHA-256'],['sha384','SHA-384']]))}${srow(t('set_cipher'),t('set_cipher_d'),certsel('cert_cipher_policy',[['lax',t('ci_lax')],['standard',t('ci_std')],['strict',t('ci_strict')],['custom',t('ci_custom')]]))}${famChips}<div class="sd2" style="margin:10px 0 2px;color:var(--tx3)">${t('set_dev_thr')}</div>${srow(t('set_cpu_hi'),t('set_cpu_hi_d'),devsel('dev_cpu_hi',[['75','75%'],['80','80%'],['85','85%'],['90','90%'],['95','95%']]))}${srow(t('set_ram_hi'),t('set_ram_hi_d'),devsel('dev_ram_hi',[['75','75%'],['80','80%'],['85','85%'],['90','90%'],['95','95%']]))}${srow(t('set_temp_hi'),t('set_temp_hi_d'),devsel('dev_temp_hi',[['70','70°C'],['75','75°C'],['80','80°C'],['85','85°C'],['90','90°C']]))}</div>`;
;
...
function vStack(d){const cs=d.containers||[];const up=c=>/^Up\b/.test(c.status||'');
 ...
 const chans=(si.channels||[]).map(c=>`<span class="tag">${esc(c)}</span>`).join('')||'—';
 ...
 const toolrows=agents.map(a=>`<div class="kv"><span class="k">${a[1]}</span><span class="v"><button class="btn" data-act="sysdo" data-do="doctor" data-sb="${a[0]}">${t('sys_doctor')}</button> <button class="btn" data-act="sysdo" data-do="logs" data-sb="${a[0]}">${t('sys_logs')}</button></span></div>`).join('');
 const toolcard=isadm?`<div class="card"><div class="ct"><span class="ico" style="background:var(--card2);color:var(--accent)">🩺</span>${t('sys_tools')}</div><div class="sd2" style="margin-bottom:6px;line-height:1.6">${t('sys_tools_d')}</div>${toolrows}<div class="kv"><span class="k">${t('sys_global')}</span><span class="v"><button class="btn" data-act="sysdo" data-do="stale">${t('sys_stale')}</button> <button class="btn" data-act="sysdo" data-do="gsettings">${t('sys_gsettings')}</button></span></div></div>`:'';
 return `${banner}<div class="sec">${t('stk_services')}</div>${svc}<div class="sec">${t('sys_section')}</div><div style="display:flex;flex-direction:column;gap:16px">${infcard}${gwcard}${metacard}${toolcard}</div><div class="sec">${t('stk_containers')} · ${upn}/${tot} ${t('stk_running')}</div><div style="display:flex;flex-direction:column;gap:16px">${cont}</div>`}
...
document.addEventListener('click',e=>{
 ...
 else if(act==='sysdo'){const dd=b.dataset.do,sb=b.dataset.sb||'',old=b.textContent;b.disabled=true;b.textContent=t('running');sysShow(dd+(sb?(' · '+sb):''),t('running')+'…');fetch('/api/sys?do='+encodeURIComponent(dd)+(sb?('&sb='+encodeURIComponent(sb)):''),{cache:'no-store'}).then(r=>r.json()).then(r=>sysShow(r.title||dd,r.out||'(empty)')).catch(()=>toast(t('toast_fail'))).finally(()=>{b.disabled=false;b.textContent=old})}
    ...
    def do_GET(self):
        ...
        if p == "/api/policy-ro":   # 唯讀:任一 agent 的 live 政策(治理頁唯讀卡 agent 選單)
            ...
            try:
                ...
                return self._send(500, json.dumps({"ok": False, "msg": str(e)}), "application/json")
        if p == "/api/sys":   # 中價值診斷(on-demand;admin):doctor / logs / stale / gsettings
            if sess["role"] != "admin":
            ...
            try:
                return self._send(200, json.dumps(do_sys(do, sb, tail), ensure_ascii=False), "application/json; charset=utf-8")
            except Exception as e:
