...
# (CONNECT 10.200.0.1:3128 → openclaw_bridge policy → X-Bridge-Token → OpenClaw :9099),
# 然後等修復完成、驗收 marker、印出 OPA ALLOWED 治理 log。
# 成本:1 個 Azure turn(openclaw agent 實修)。
...
[ -n "$CT_H" ] && [ -n "$CT_O" ] || bad "容器未找到(先 bash scripts/boot-stack.sh)"
...
[ -n "$OCIP" ] || bad "拿不到 my-assistant IP"
...
last(){ docker exec "$CT_O" sh -c "curl -s -m5 -H 'X-Bridge-Token: $TOKEN' http://127.0.0.1:9099/last" 2>/dev/null; }
...
ACK=$(docker exec -u 0 "$CT_H" ip netns exec "$NS" su -s /bin/bash -c \
...
{ [ -n "$TS" ] && [ "$TS" != "$PREV_TS" ]; } || bad "等不到新修復結果(看 docker exec $CT_O cat /tmp/openclaw-fix-endpoint.log)"
...
echo "  OPA 治理 log:"
docker logs --since 10m "$CT_H" 2>&1 | grep -a "$OCIP:9099" | tail -2 | sed 's/^/    /' || true
